...
#inner_join(patients, visits) also works

all_visits <- left_join(patients, visits)

full_join(visits,patients)

vd <- full_join(visits, doctors)

#pd <- anti_join(patients, doctors)
       #filter(pd, patient$name == 'Bob' )


anti_join(visits, patients) #who hasn't come
```
...
Add origin and destination airport names (two joins to the same table) Question: Which origin–destination pair has the highest average arrival delay?
...
Plane metadata join: age of aircraft vs. delays Among aircraft with at least 50 flights, which tailnumber has the largest average arrival delay, and what are the manufacturer/model and age?
...
What does anti_join(flights, airports, by = c("dest" = "faa")) tell you? What does anti_join(airports, flights, by = c("faa" = "dest")) tell you? Perform the two joins below and consider what the results **are**.
